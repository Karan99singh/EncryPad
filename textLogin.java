		...
		public class Listenobj implements ActionListener{
			public void actionPerformed(ActionEvent event) {
				...
				if(event.getSource()==login) {
					...
					if (file.exists()==true){
						try {
							Decrypt.Decrun(textMain.Defaultkey, "pass", file.getPath());
						} catch (Exception e1) {
						...
						if(checkpass.contentEquals(String.valueOf(passwordtext.getPassword()))==true) {
							try {
								userSession START = new userSession(file.getPath(),checkpass);
						...
						try {
							Formatter format = new Formatter(String.format("%s\\pass.txt",file.getPath()));
							format.close();
						} catch (FileNotFoundException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
